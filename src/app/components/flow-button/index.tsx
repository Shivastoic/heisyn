...
export default function FlowButton({ text = "Modern Button" }: { text?: string }) {
    return (
        <button className="group relative flex items-center gap-1 overflow-hidden rounded-[100px] border-2 border-neutral-100 bg-transparent px-8 py-3 text-sm font-medium font-jetbrainsmono text-neutral-100 cursor-pointer transition-all duration-[600ms] ease-[cubic-bezier(0.23,1,0.32,1)] hover:border-transparent hover:text-black hover:rounded-[12px] active:scale-[0.95]">
            {/* Left arrow (arr-2) */}
            <FaArrowRight 
